...
/*    06/09/2020(TimS.):    Updated for FreeRTOS with Vitis 2020.1            */
/*                                                                            */
/******************************************************************************/
...
*
******************************************************************************/
XStatus SF3_freertos_begin(PmodSF3_freertos *InstancePtr, const u32 SPI_Address, const u32 Intc_Vec_Id, const u32 Intc_Intr) {
   XStatus Status;
   const ivt_t ivt[] = {
      {Intc_Vec_Id, (XInterruptHandler) XSpi_InterruptHandler, &(InstancePtr->SF3Spi)}
   };

   XSpi_SF3Config.BaseAddress = SPI_Address;

   // Initialize SPI
   Status = SF3_freertos_SpiInit(&InstancePtr->SF3Spi);
   ...
   //EnableInterrupts(&(InstancePtr->sIntc), &ivt[0], 1);

   /* Disabled as no direct register intc writes are compatible with FreeRTOS. */
...
*
******************************************************************************/
XStatus SF3_freertos_SpiInit(XSpi *SpiPtr) {
   XStatus Status;
   ...
   XSpi_SetStatusHandler(SpiPtr, SpiPtr, (XSpi_StatusHandler) SF3_freertos_SpiHandler);
...
*
******************************************************************************/
XStatus SF3_freertos_FlashWriteEnable(PmodSF3_freertos *InstancePtr) {
   XStatus Status;
   u8 Buffer[SF3_WRITE_ENABLE_BYTES];

   Status = SF3_freertos_WaitForFlashReady(InstancePtr);
   if (Status != XST_SUCCESS) {
      return XST_FAILURE;
   }
...
*
******************************************************************************/
XStatus SF3_freertos_FlashWrite(PmodSF3_freertos *InstancePtr, u32 Addr, u32 ByteCount,
...
*
******************************************************************************/
XStatus SF3_freertos_FlashRead(PmodSF3_freertos *InstancePtr, u32 Addr, u32 ByteCount, u8 ReadCmd,
      u8 **BufferPtr) {
   XStatus Status;
   u32 DummyBytes;
...
*
******************************************************************************/
XStatus SF3_freertos_BulkErase(PmodSF3_freertos *InstancePtr) {
   XStatus Status;
...
*
******************************************************************************/
XStatus SF3_freertos_SectorErase(PmodSF3_freertos *InstancePtr, u32 Addr) {
   XStatus Status;
...
/******************************************************************************
* This function erases the contents of an 4KB subsector.
*
* @param    InstancePtr is a pointer to the instance of the PmodSF3 device.
* @param    Addr is the address of the first byte in the sector to be erased.
*
...
*
******************************************************************************/
XStatus SF3_freertos_SubsectorErase(PmodSF3_freertos *InstancePtr, u32 Addr) {
   XStatus Status;
   u8 Buffer[SF3_SUBSECTOR_ERASE_BYTES];
   ...
   Buffer[0] = SF3_COMMAND_SUBSECTOR_ERASE;
   Buffer[1] = (u8) (Addr >> 16);
   Buffer[2] = (u8) (Addr >> 8);
   Buffer[3] = (u8) (Addr);
...
*
******************************************************************************/
XStatus SF3_freertos_GetStatus(PmodSF3_freertos *InstancePtr) {
   XStatus Status;

   u8 BufferWr[SF3_STATUS_READ_BYTES];
   u8 BufferRd[SF3_STATUS_READ_BYTES];

   BufferWr[0] = SF3_COMMAND_READ_STATUSREG;
   BufferWr[1] = 0xFF;
   BufferRd[0] = 0xFF;
   BufferRd[1] = 0xFF;

   TransferInProgress = TRUE;
   Status = XSpi_Transfer(&InstancePtr->SF3Spi, BufferWr, BufferRd,
...
*
******************************************************************************/
XStatus SF3_freertos_WaitForFlashReady(PmodSF3_freertos *InstancePtr) {
   XStatus Status;
   u8 StatusReg;
   ...
   while (1) {
       ...
       */
      Status = SF3_freertos_GetStatus(InstancePtr);
      if (Status != XST_SUCCESS) {
         return XST_FAILURE;
      }
...
*
******************************************************************************/
XStatus SF3_freertos_ReadID(PmodSF3_freertos *InstancePtr) {
   XStatus Status;
   int i;
...
*
******************************************************************************/
void SF3_freertos_SpiHandler(void *CallBackRef, u32 StatusEvent,
